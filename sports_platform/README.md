...
The Admin Panel (`AdminPanelScreen`) allows managers to:
1. **Enable/Disable** the sports feature globally.
2. **Link Live Streams**: Associate a fixture ID from API-Football with a direct stream URL (HLS/WebRTC).
3. **Manage Status**: Monitor active streams and remove ended matches.

## Video Engine (C++) Integration

The Flutter app utilizes a high-performance C++ video engine bridge.
- **Implementation**: The `SportsVideoPlayer` widget uses the `media_kit` package, which is a wrapper around `libmpv` (written in C++).
- **Format**: Supports HLS, DASH, and low-latency WebRTC streams generated by the `video_engine_cpp`.
- **Optimization**: Uses hardware acceleration (NVENC/VAAPI) where available on the device for 4K streaming with low CPU impact.
